...
# Lister et sélectionner une voix française si disponible
for voix in engine.getProperty('voices'):
    if "french" in voix.name.lower() or "fr" in voix.id.lower():
        engine.setProperty('voice', voix.id)
        break

engine.save_to_file(texte, 'narration_HSE.mp3')
engine.runAndWait()
